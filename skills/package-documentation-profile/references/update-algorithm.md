...
Set manifest `source.git.current_hash` to the committed target revision. Set `previous_hash` to the prior manifest target for incremental runs. Record `working_tree_dirty` and dirty files without changing the source hash. Hash exact bytes for completion-result input, output, and preserved paths. Return the declared completion envelope with profile stage, `package-profile.generate` capability, requested behavior, warnings and preserved module paths. Its textbook target arrays are empty and concept metrics are zero. Requested source scope and effective module/facet scope are carried by the digested invocation and manifest, not extra result fields.
