...
def calculate_cis(nodes: List[dict], edges: List[dict]) -> Dict[int, int]:
    ...
    for edge in edges:
        ...
        if key[0] not in known or key[1] not in known:
            raise ValueError(f"Dependency references unknown concept: {key}")
        if key in edge_keys:
    ...
    if len(cis) != len(node_ids):
        ...
        raise ValueError(
            ...
            f"{'...' if len(unscored) > 10 else ''}). Run ibook graph validate "
            f"to verify DAG structure before retrying."
        )

    return cis


def csv_to_json(csv_path: str, color_config: dict = None,
    ...
    for tax_id in sorted(used_taxonomies):
