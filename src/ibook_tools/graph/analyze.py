...
def generate_report(csv_path: str, output_path: str):
    ...
    if not concepts:
        raise ValueError("Cannot analyze an empty graph")
    known = set(concepts)
    for concept, prereqs in dependencies.items():
        if len(prereqs) != len(set(prereqs)) or any(p not in known for p in prereqs):
            raise ValueError(f"Invalid prerequisites for concept {concept}")
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write("\n---\n\n")
        f.write("*Report generated by ibook graph analyze*\n")
